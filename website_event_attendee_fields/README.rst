...
By default ``website_event`` module asks only three fields to fill in the
...
-  If user is authenticated:
   ...
   -  the first attendee will have autofilled values at the form (if person
...
-  Modifies behaviour of ``partner_event`` module:
   ...
   -  If attendee partner exists and current (authenticated) user is the
      ...
      fields every time, because it leads to security issue: anyone can
...
-  Custom redirection after filling ticket form, e.g. to the cart page to
...
-  Fill in the fields and proceed checkout
-  Go back to backend. Open menu ``Events``
...
-  When email column is presented and there is a partner with that
   ...
   -  if partner has some of fields
      ...
      -  the following message is shown: "This email address already has an
         account. Data will be taken from this account"

-  The registration form doesn't allow to register two attendees with the
   same email

-  When partner record exists before purchasing the module and the current
